...
case class Building(
  ...
  def apportionedTwoPercent(apStartDate: LocalDate, apEndDate: LocalDate) = getSBADetails(apStartDate, apEndDate, nonResidentialActivityStart, cost).flatMap(_.totalCost).getOrElse(0)
