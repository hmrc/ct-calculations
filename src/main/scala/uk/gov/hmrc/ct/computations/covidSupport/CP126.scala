...
object CP126 extends Calculated[CP126, ComputationsBoxRetriever] {
  override def calculate(boxRetriever: ComputationsBoxRetriever): CP126 = {
    ...
    val cp125a = boxRetriever.cp125a()
    val cp125b = boxRetriever.cp125b()

    CP126(cp122.value.getOrElse(0) - cp123.value.getOrElse(0) - cp124.value.getOrElse(0) + cp125a.value.getOrElse(0) + cp125b.value.getOrElse(0))
  }
}