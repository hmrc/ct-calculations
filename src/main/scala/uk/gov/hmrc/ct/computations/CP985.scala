...
object CP985 extends Calculated[CP985, ComputationsBoxRetriever] {
  ...
  override def calculate(boxRetriever: ComputationsBoxRetriever): CP985 = {
    CP985(
      boxRetriever.cp500.value - boxRetriever.cp983.value.getOrElse(0) + boxRetriever.cp981.value.getOrElse(0) + boxRetriever.cp980.value.getOrElse(0) + boxRetriever.cp982.value.getOrElse(0)
    )
  }
}
