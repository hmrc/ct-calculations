...
class NorthernIrelandRateValidationSpec extends WordSpec with Matchers with MockitoSugar {
  ...
  "NorthernIrelandRateValidation" should {
    "if NIR is active for current period with NIR losses carried forward from previous period" when {
      ...
      val table = Table(
        ("message",                                                         "CP117",    "CATO01",       "cpq17",        "allLossesBroughtForward: Total      pre      post        NI_Loss     Main_Loss",  "lossesBroughtForwardAgainstTradingProfit: Total      pre      post        NI_Loss     Main_Loss",   "cp284",     "cpnewbox",     "cpnewboxa","cpnewboxb", "lossesBroughtForwardAgainstNonTradingProfit: Total       NI_Loss    Main_Loss   NI_Loss_Revalued"),
