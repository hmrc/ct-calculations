  ...
  "CPQ18" should {
    ...
    "when empty" when {
      "fail validation when CPQ17 is false and CPQ19 is not defined" in new SetUpboxRetriever {
        ...
        when(retriever.retrieveCPQ17()).thenReturn(CPQ17(Some(false)))
      ...
      "fail validation when CPQ19 is false and CPQ17 is not defined" in new SetUpboxRetriever {
        ...
        when(retriever.retrieveCPQ19()).thenReturn(CPQ19(Some(false)))
        CPQ18(None).validate(retriever) shouldBe Set(CtValidation(Some("CPQ18"), "error.CPQ18.required"))
      }
...
trait SetUpboxRetriever extends MockitoSugar {
  ...
  def setUpRetriever() = {
    ...
    when(retriever.retrieveCPQ17()).thenReturn(CPQ17(None))

    when(retriever.retrieveCPQ18()).thenReturn(CPQ18(None))

    when(retriever.retrieveCPQ19()).thenReturn(CPQ19(None))

    when(retriever.retrieveCPQ20()).thenReturn(CPQ20(None))

    when(retriever.retrieveCP117()).thenReturn(CP117(0))

    when(retriever.retrieveCP118()).thenReturn(CP118(0))

    when(retriever.retrieveCP284()).thenReturn(CP284(None))

    when(retriever.retrieveCATO01()).thenReturn(CATO01(0))
  }
