...
class CP677Spec extends AnyWordSpec with MockitoSugar with Matchers with BeforeAndAfter {
  ...
  "CP677" should{
    "Calculate the correct value" in {
      when(boxRetriever.cp1()).thenReturn(CP1(LocalDate.of(2022,10,1)))
      when(boxRetriever.cp2()).thenReturn(CP2(LocalDate.of(2023,9,30)))
    ...
    "Not Calculate when dates outside super deduction period" in {
      when(boxRetriever.cp1()).thenReturn(CP1(LocalDate.of(1993,10,1)))
      when(boxRetriever.cp2()).thenReturn(CP2(LocalDate.of(1994,9,30)))
      when(boxRetriever.cp675()).thenReturn(CP675(None))
      CP677.calculate(boxRetriever) shouldBe CP677(None)
    }
  }
