...
class AC12Spec extends AnyWordSpec with Matchers with MockitoSugar {
  ...
  "AC12 validation" should {
    ...
    "zeroOrPositive check" when {
      val testValues = Set(
        (Some(-1), "and return an error if negative"),
        (Some(0), "and return no error if zero"),
        (Some(1), "and return no error if positive")
      )
