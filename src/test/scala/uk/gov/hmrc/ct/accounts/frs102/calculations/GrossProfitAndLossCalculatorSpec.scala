  ...
  "GrossProfitAndLossCalculator" should {
    ...
    "calculate AC24 whilst going through the Full journey" when {
      "all inputs are empty, AC24 is empty" in new GrossProfitAndLossCalculator {
        calculateAC24Full(AC12(None), noGrossTurnoverFromOPW, AC14(None), noDeductionsFromOPW) shouldBe AC24(None)
      }
      "AC12 and AC401 have a value and value and AC14 and AC403 are empty, AC24 is positive"in new GrossProfitAndLossCalculator {
        calculateAC24Full(AC12(Some(12)), AC401(Some(10)), AC14(None), noDeductionsFromOPW) shouldBe AC24(Some(22))
      }
      "AC12 and AC401 are empty and AC14 and AC403 have value, AC24 is negative" in new GrossProfitAndLossCalculator {
        calculateAC24Full(AC12(None), noGrossTurnoverFromOPW, AC14(Some(14)), AC403(16)) shouldBe AC24(Some(-30))
      }
      "all the boxes have value, calculate AC24 correctly" in new GrossProfitAndLossCalculator {
        calculateAC24Full(AC12(Some(12)), AC401(Some(20)), AC14(Some(14)), AC403(6)) shouldBe AC24(Some(12))
      }
    }

    "calculate AC24 whilst going through the Abridged journey" when {
      "all inputs are empty, AC24 is empty" in new GrossProfitAndLossCalculator { // isn't this test superfluous as the opw boxes are mandatory???
        calculateAC24Abridged(AC16(None), noGrossTurnoverFromOPW, noDeductionsFromOPW) shouldBe AC24(None)
      }
      "return AC12 value as AC24 if AC12 and AC401 has value and AC14 and AC403 are empty" in new GrossProfitAndLossCalculator {
        calculateAC24Abridged(AC16(Some(12)), AC401(Some(10)), noDeductionsFromOPW) shouldBe AC24(Some(22))
      }
      "all the boxes have value, calculate AC24 correctly" in new GrossProfitAndLossCalculator {
        calculateAC24Abridged(AC16(Some(12)), AC401(Some(20)), AC403(9)) shouldBe AC24(Some(32))
      }
    }
    ...
    "calculate AC25 whilst going through the Full journey" when {
      "all inputs are empty, AC25 is empty" in new GrossProfitAndLossCalculator {
        calculateAC25Full(AC13(None), AC402(None), AC15(None), noPreviousDeductionsFromOPW) shouldBe AC25(None)
      }
      "AC13 and AC402 have value and AC15 is empty, add AC13 and AC402" in new GrossProfitAndLossCalculator {
        calculateAC25Full(AC13(Some(13)), AC402(Some(8)), AC15(None), noPreviousDeductionsFromOPW) shouldBe AC25(Some(21))
      }
    ...
    "calculate AC25 whilst going through the Abridged journey" when {
      "all inputs are empty, AC25 is empty" in new GrossProfitAndLossCalculator {
        calculateAC25Abridged(AC17(None), AC402(None), noPreviousDeductionsFromOPW) shouldBe AC25(None)
      }
