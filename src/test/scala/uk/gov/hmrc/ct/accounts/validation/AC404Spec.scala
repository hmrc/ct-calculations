...
class AC404Spec extends WordSpec with Matchers with MockitoSugar{
  ...
  "AC404 validation" should {
    ...
    "show error if AC404 is present when AC402 doesnt Exist" in {
      when(boxRetriever.ac402())thenReturn AC402(None)
      val validationResult = AC404(1).validate(boxRetriever)

      validationResult shouldBe Set(CtValidation(Some("AC404"),"error.AC404.cannot.exist"))
