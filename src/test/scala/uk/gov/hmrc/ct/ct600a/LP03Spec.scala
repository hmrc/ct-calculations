...
class LP03Spec extends AnyWordSpec with Matchers {
  ...
  "LP03 to json" should {
    "create valid json for a single writeoff using only required fields " in {
      val lp03 = LP03(writeOffs = Some(List(WriteOff("123", 1, LocalDate.of(2012,3,1)))))
    ...
    "create valid json for multiple loans" in {
      val lp03 = LP03(writeOffs =  Some(List(WriteOff("123", 1, LocalDate.of(2012,3,1)), WriteOff("456", 1, LocalDate.of(2014,3,1))) ))
    ...
    "be added to another LP03" in {
      val lp03a = LP03(writeOffs = Some(List(WriteOff("123", 1, LocalDate.of(2014,4,4)))))
      val lp03b = LP03(writeOffs = Some(List(WriteOff("456", 1, LocalDate.of(2012,3,1)))))
      val expected = LP03(writeOffs = Some(List(WriteOff("123", 1, LocalDate.of(2014,4,4)), WriteOff("456", 1, LocalDate.of(2012,3,1)))))
  ...
  "LP03 from json" should {
    "create LP03 with single write off " in {
      Loans.lp03FromJsonString("""{"writeOffs":[{"loanId":"123","amountWrittenOff":1,"dateWrittenOff":"2012-03-01"}]}""") shouldBe LP03(writeOffs = Some(List(WriteOff("123", 1, LocalDate.of(2012,3,1)))))
    }
    "create LP03 with multiple write offs " in {
      val expected = LP03(Some(List(WriteOff("123", 1, LocalDate.of(2012,3,1)), WriteOff("456", 1, LocalDate.of(2014,3,1)))))
